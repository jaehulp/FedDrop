...
for i, ep in enumerate(e):
    ...
    svd_list, server_singular, client_singular, singular_diff = measure_client_raw_svd_block3(epoch, model_dir, data_dir, model_dict, model_list, idx_list)
    ...
    plt.scatter(server_weightsum, drop_acc1)
    plt.xlabel('WeightSum SVD Corr Rank')
    plt.ylabel('Drop Acc1 diff')
    plt.grid(True)
    plt.title(f'SvdCorr vs Drop acc1 Block4 epoch{epoch}(Server singular Val)')
    plt.savefig(f'./graph_img2/DropAcc_WeightSumSVD_server_blk4_{epoch}_ResNet18.png', dpi=300)
    plt.clf()
