...
logger.addHandler(logging.FileHandler(os.path.join(log_dir, 'log.txt'), mode='w'))
...
for i in range(2, 300):
    ...
    cov_index = np.argsort(cov_mean_list)
    svd_index = np.argsort(svd_cov_mean_list)
    client_acc_list, cov_mean_list, svd_cov_mean_list, topk_svd_mean_list, dis_list = measure_client_testset(epoch, model_dir, data_dir, testloader, model_dict, model_list, idx_list)

    logger.info(f'test dataset')
    logger.info(f'client_acc_list {client_acc_list}')
    logger.info(f'cov_mean_list {cov_mean_list}')
    logger.info(f'svd_cov_mean_list {svd_cov_mean_list}')
    logger.info(f'topk_svd_mean_list {topk_svd_mean_list}')
    logger.info(f'topk_svd_mean_list {topk_svd_mean_list}')
    ...
    idx = np.argsort(client_acc_list)
    idx = idx[2:]
    norm_weight = weight[idx]
    norm_weight = norm_weight / sum(norm_weight)
    new_list = [model_list[i] for i in idx]
