...
acc1_diff = torch.cat((acc1_default[1:], torch.tensor([0]))) - acc1_default
acc1_diff = acc1_diff[:-1]

cov_diff = torch.cat((cov_mean[1:], torch.tensor([0]))) - cov_mean
cov_diff = cov_diff[:-1]

acc1_diff = acc1_diff > 0
cov_diff = cov_diff > 0

print(sum(acc1_diff))
print(sum(cov_diff))
a = torch.logical_xor(acc1_diff, cov_diff)
print(sum(a))