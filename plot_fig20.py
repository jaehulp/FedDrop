...
plt.title('SvdCorr Rank vs Drop acc1 (Server singular Val)')
plt.grid(True)
...
for i in e:
    ...
    svd_list, server_singular, client_singular, singular_diff = measure_client_raw_svd_block4(epoch, model_dir, data_dir, model_dict, model_list, idx_list)
    ...
    plt.scatter(server_weightsum, Ssorted_drop_acc1, color='blue')
plt.savefig(f'./graph_img/DropAccRank_WeightSumSVD_server_10epochs_blk4_ResNet18.png', dpi=300)
plt.clf()
