...
disagreement = torch.mean(disagreement_mat, dim=1)
...
disagreement_index = torch.argsort(disagreement_mat, dim=1)
sorted_acc1_client_testset= torch.gather(acc1_client_testset, dim=1, index=disagreement_index)
...
plt.grid(True)
plt.legend()
plt.savefig('./graph_img/DisaggArgsort_ClientTestsetAcc_ResNet18.png', dpi=300)
